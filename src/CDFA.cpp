                                    ...
                                    std::unordered_set<StateId>& final_states, StateId sink_id)
{
    ...
    for (const auto& current_state_id : nfa.GetStatesId())
    {
        ...
        if (composit_states_info.contains(current_state_id)) // that means current_state_id is composit one
            continue;                                        // and should be processed later

        for (const auto& [destination_state_id, strings] : nfa.GetTransitions().at(current_state_id))
        {
            for (const auto& string: strings)
            {
                transitions_table[current_state_id][string[0]].insert(destination_state_id);
            }
        }

        for (const auto& symbol : nfa.GetAlphabet())
        {
            const auto& destinations_by_symbol = transitions_table[current_state_id][symbol];
            ...
            if (destinations_by_symbol.size() > 1)
            {
                ...
                auto new_composit_state_id = composit_destinations_info[destinations_by_symbol];

                nfa.AddTransition({{current_state_id, new_composit_state_id}, symbol});

                for (const auto& destination: destinations_by_symbol)
                {
                    nfa.RemoveTransition({{current_state_id, destination}, symbol}); // removing old transitions
                }
            }
        }
    }
    ...
    while(!queue_for_procession.empty())
    {
        ...
        for (const auto& symbol : nfa.GetAlphabet())
        {
            ...
            if (destinations_by_symbol.size() >= 2)
            {
                ...
                {
                    auto new_composit_state_id = AddCompositState(nfa, destinations_by_symbol, final_states,
                                                         composit_states_info, composit_destinations_info);
                    queue_for_procession.push_back(new_composit_state_id);

                    nfa.AddTransition({{current_composit_state, new_composit_state_id}, symbol});
                }
            }
        }
    }
