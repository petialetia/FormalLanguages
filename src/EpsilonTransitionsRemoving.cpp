...
void MakeEpsilonTransitiveClosure(
    NFA& nfa, std::unordered_set<Edge, EdgeHash> epsilon_transitions) {
START_OVER:

  for (auto first_edge_iterator = epsilon_transitions.cbegin();
       first_edge_iterator != epsilon_transitions.cend();
       ++first_edge_iterator) {
    for (auto second_edge_iterator = epsilon_transitions.cbegin();
         second_edge_iterator != epsilon_transitions.cend();
         ++second_edge_iterator) {
      if (((*first_edge_iterator).destination_ ==
           (*second_edge_iterator).start_) &&
          (!epsilon_transitions.contains(
              {(*first_edge_iterator).start_,
               (*second_edge_iterator).destination_}))) {
        Edge new_transition_edge = {(*first_edge_iterator).start_,
                                    (*second_edge_iterator).destination_};
...
START_OVER:
  ...
  for (const auto state_id : nfa.GetStatesId()) {
    if (final_states.contains(state_id)) continue;

    for (const auto& [one_step_reachable_state, strings] :
         transitions.at(state_id)) {
      if (final_states.contains(one_step_reachable_state) &&
          strings.contains(EPSILON)) {
...
START_OVER:
  ...
  for (auto epsilon_edge_iterator = epsilon_transitions.cbegin();
       epsilon_edge_iterator != epsilon_transitions.cend();
       ++epsilon_edge_iterator) {
    bool is_start_over_needed = false;

    for (const auto& [one_step_reachable_state, strings] :
         nfa[(*epsilon_edge_iterator).destination_]) {
      for (const auto& string : strings) {
        Transition new_transition = {
            {(*epsilon_edge_iterator).start_, one_step_reachable_state},
            string};
    ...
    if (is_start_over_needed) goto START_OVER;
  }
}
...
void DeleteUnreachableStates(NFA& nfa) {
  ...
  for (auto state : states) {
    if (!reachable_states.contains(state)) nfa.DeleteState(state);
  }
}