...
bool StateAndNeighboursClasses::operator==(
    const StateAndNeighboursClasses& other) const {
  return class_id_ == other.class_id_ &&
         neighbours_classes_ == other.neighbours_classes_;
}

std::size_t StateAndNeighboursClassesHash::operator()(
    const StateAndNeighboursClasses& classes) const {
  std::size_t seed = 0;
...
void ChangeToMinimalCDFA(NFA& nfa) {
  ...
  while (GetNumOfClasses(current_stage_classes) !=
         GetNumOfClasses(next_stage_classes)) {
...
StateClassInfos GetNextStageClasses(
  ...
  std::unordered_map<StateId, std::vector<ClassId>>
      class_to_class_transition_table;
  ...
  for (const auto& state : nfa.GetStatesId()) {
    for (const auto& letter : nfa.GetAlphabet()) {
      ...
      class_to_class_transition_table[state].push_back(
          current_stage_classes.at(*neighbour_state.cbegin()));
    }
  }
  ...
  std::unordered_map<StateAndNeighboursClasses, ClassId,
                     StateAndNeighboursClassesHash>
      new_classes;

  for (const auto& state : nfa.GetStatesId()) {
    StateAndNeighboursClasses nearest_classes = {
        .class_id_ = current_stage_classes.at(state),
