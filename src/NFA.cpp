...
Edge::Edge(StateId start, StateId destination)
...
Transition::Transition(Edge edge, std::string string)
    : edge_(edge), string_(string) {}

Transition::Transition(Edge edge, char symbol)
    : edge_(edge), string_(1, symbol) {}
...
bool NFA::IsValid() const { return start_state_.has_value(); }
...
void NFA::AddTransition(Transition transition) {
  ...
  transitions_[transition.edge_.start_][transition.edge_.destination_].insert(
      transition.string_);
}

bool NFA::IsTransition(const Transition& transition) const {
  if (!transitions_.contains(transition.edge_.start_)) return false;

  if (!transitions_.at(transition.edge_.start_)
           .contains(transition.edge_.destination_))
    return false;

  return transitions_.at(transition.edge_.start_)
      .at(transition.edge_.destination_)
      .contains(transition.string_);
}

const std::set<StateId>& NFA::GetStatesId() const { return states_id_; }
...
void NFA::DeleteState(StateId state_id) {
  ...
  if (start_state_.has_value()) {
    if (start_state_.value() == state_id) start_state_ = std::nullopt;
  }
}

const TransitionsStorage& NFA::GetTransitions() const { return transitions_; }

void NFA::RemoveTransition(Transition transition) {
  if (transitions_.contains(transition.edge_.start_) &&
      transitions_[transition.edge_.start_].contains(
          transition.edge_.destination_) &&
      transitions_[transition.edge_.start_][transition.edge_.destination_]
          .contains(transition.string_)) {
    transitions_[transition.edge_.start_][transition.edge_.destination_].erase(
        transition.string_);
  }
}

std::unordered_map<StateId, std::unordered_set<std::string>>& NFA::operator[](
    StateId id) {
...
std::unordered_set<StateId> GetDestinationsByString(const NFA& nfa,
  ...
  for (const auto& [destination, transition_strings] :
       nfa.GetTransitions().at(start)) {
