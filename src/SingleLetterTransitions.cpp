...
void MakeTransitionsSingleLetter(NFA& nfa)
{
    ...
    for (const auto& [start_state_id, transitions_from_start_state] : nfa.GetTransitions())
    {
        for (const auto& [destination_state_id, strings] : transitions_from_start_state)
        {
            for (const auto& string : strings)
            {
                if (string.length() >= 2)
                {
                    transitions_to_remove.insert({{start_state_id, destination_state_id}, string});
                    ...
                    transitions_to_add.insert({{start_state_id, *added_states_iterator}, std::string(1, string[0])});

                    for (size_t i = 1; i < string.length() - 1; ++i, ++added_states_iterator)
                    {
                        transitions_to_add.insert({{*added_states_iterator, *std::next(added_states_iterator, 1)}, std::string(1, string[i])});
                    }
