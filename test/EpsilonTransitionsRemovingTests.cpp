...
TEST(GetEpsilonTransitions, Correctness) {
  ...
  EXPECT_FALSE(
      epsilon_transitions.contains({second_state_id, second_state_id}));
}
...
TEST(MakeEpsilonTransitiveClosure, Correctness1) {
  ...
  auto start_to_start_transitions_iterator =
      (*start_state_transitions_iterator).second.find(start_state_id);
  EXPECT_FALSE(start_to_start_transitions_iterator ==
               (*start_state_transitions_iterator).second.end());

  auto start_to_second_transitions_iterator =
      (*start_state_transitions_iterator).second.find(second_state_id);
  EXPECT_FALSE(start_to_second_transitions_iterator ==
               (*start_state_transitions_iterator).second.end());

  auto second_to_start_transitions_iterator =
      (*second_state_transitions_iterator).second.find(start_state_id);
  EXPECT_FALSE(second_to_start_transitions_iterator ==
               (*second_state_transitions_iterator).second.end());

  auto second_to_second_transitions_iterator =
      (*second_state_transitions_iterator).second.find(second_state_id);
  EXPECT_FALSE(second_to_second_transitions_iterator ==
               (*second_state_transitions_iterator).second.end());
  ...
  EXPECT_TRUE(
      (*second_to_second_transitions_iterator).second.contains(EPSILON));
}
...
TEST(MakeEpsilonTransitiveClosure, Correctness2) {
  ...
  EXPECT_TRUE(
      transitions.at(start_state_id).at(start_state_id).contains(EPSILON));
  EXPECT_TRUE(
      transitions.at(start_state_id).at(second_state_id).contains(EPSILON));
  EXPECT_TRUE(
      transitions.at(second_state_id).at(start_state_id).contains(EPSILON));
  EXPECT_TRUE(
      transitions.at(second_state_id).at(second_state_id).contains(EPSILON));
  EXPECT_TRUE(
      transitions.at(start_state_id).at(third_state_id).contains(EPSILON));
  EXPECT_TRUE(
      transitions.at(second_state_id).at(third_state_id).contains(EPSILON));
...
TEST(AddTransitions, Correctness1) {
  ...
  EXPECT_TRUE(
      transitions.at(start_state_id).at(fouth_state_id).contains("abc"));

  EXPECT_TRUE(transitions.at(second_state_id).contains(fouth_state_id));
  EXPECT_TRUE(
      transitions.at(second_state_id).at(fouth_state_id).contains("abc"));
}
