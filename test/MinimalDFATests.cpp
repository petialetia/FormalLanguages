...
TEST(ChangeToMinimalDFA, Correctness)
{
    ...
    for (const auto& [destination, strings] : nfa.GetTransitions().at(new_start))
    {
        if (!strings.empty())
        {
    ...
    for (const auto& [destination, strings] : nfa.GetTransitions().at(second_state.value()))
    {
    ...
    for (const auto& [destination, strings] : nfa.GetTransitions().at(final_state.value()))
    {
        if (!strings.empty())
        {
            EXPECT_EQ(strings.size(), 2);
    ...
    for (const auto& [destination, strings] : nfa.GetTransitions().at(sink_state.value()))
    {
        if (!strings.empty())
        {
            EXPECT_EQ(strings.size(), 2);
